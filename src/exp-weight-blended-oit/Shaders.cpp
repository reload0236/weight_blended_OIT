...
const std::string WeightBlendedOitExample::oit_output_fs =
    ...
    "uniform vec3 view_range; // .x for near, .y for far, .z for 500 / |far - near|\n"
    "\n"
    "void main(void)\n"
    "{\n"
    "    vec3 normal = normalize(vs_normal);\n"
    ...
    "    // Equaltion 9\n"
    "    float mappedAbsZ = (abs(1.0 / gl_FragCoord.w) - view_range.x) * view_range.z; \n"
    "    float minRight = 0.03 / (1e-5 + pow(mappedAbsZ / 200.0, 4.0));\n"
